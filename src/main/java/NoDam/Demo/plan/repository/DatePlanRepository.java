...
public interface DatePlanRepository extends JpaRepository<DatePlan, Long> {
    ...
    @Query("select dp from DatePlan dp left join fetch dp.placePlans where dp.id = :datePlanId")
    Optional<DatePlan> findByIdWithPlacePlans(@Param("datePlanId") Long datePlanId);
